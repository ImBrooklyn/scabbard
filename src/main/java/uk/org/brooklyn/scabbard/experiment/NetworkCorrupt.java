...
/**
 * 可以指定网卡、本地端口、远程端口、目标 IP 包损坏。需要特别注意，如果不指定端口、ip 参数，
 * 而是整个网卡包损坏，切记要添加 timeout 参数或者 excludePort 参数，前者是指定运行
 * 时间，自动停止销毁实验，后者是指定排除掉的延迟端口，两者都是防止机器无法连接的情况，如果
 * 真实发生此问题，重启机器即可恢复。
 * 本地端口和远程端口之间是或的关系，即这两个端口都会生效，只要指定了本地端口或者远程端口，
 * 无需指定需要排除的端口。端口与 IP 之间是与的关系，即指定的 IP:PORT 发生包损坏。
 *
 * @author ImBrooklyn
 * @since 10/03/2024
 */
@Experiment
@Builder
public class NetworkCorrupt extends AutoAssembledChaosExperiment {
